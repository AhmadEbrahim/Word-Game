...
class WordAStar(astar.AStar):
    ...
    def heuristic_custom(self, state):
        ...
        temp = list(state.word)  # working with the string as a list cause it is not preferred to change strings
        cost = len(temp)  # assuming that the cost to reach the goal word from the start word is the length of the start word
        # so if the whole letters are different in both start word and goal word that will result the maximum cost
        for x in range(len(temp)):  #so this loop's importance is to compare between the start word letters and the goal word letters and reduce the cost in case there is a similarities between the two words
            if temp[x] == self.gw[x]:
                cost -= 1
        return cost
    ...
    def successors(self, state):
        ...
        temp = list(state.word)  # working with the string as a list cause it is not preferred to change strings
        successors = []  # this is an array which stores in all the successors
        costs = []  # the corresponding list of costs to move to this states
        """for each letter in the start word try to change it letter by letter to reach the goal word
                so that nested loops importance is for looping on the word letter by letter and change each letter while looping on the
                the whole alphabet to change them and reach the goal word and calculate cost at the same time"""
        for i in range(len(temp)):
            temp = list(state.word)
            for j in ALPHABET:
                temp[i] = j
                strTemp = ''.join(temp)
                cost = len(temp)
                if strTemp in self.dictionary:
                    successors.append(WordState(strTemp))
                    if strTemp == self.gw:
                        costs.append(0)  # add zero if the goal word was found
                        return successors, costs
                    costs.append(1)  # append one to the cost and continue the search to find the goal word
        return successors, costs
